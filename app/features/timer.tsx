...
export default function TimerScreen() {
  ...
  // --- 2. COEUR DU TIMER (CORRECTION TYPAGE) ---
  useEffect(() => {
    if (isActive) {
      ...
      }, 1000) as unknown as NodeJS.Timeout; // FIX TYPESCRIPT ICI
  ...
  const styles = StyleSheet.create({
    ...
    tabBtn: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 20, backgroundColor: theme.colors.glass, borderWidth: 1, borderColor: theme.colors.border },
    ...
    timerCircle: { width: 250, height: 250, borderRadius: 125, justifyContent: 'center', alignItems: 'center', marginBottom: 30, marginTop: 20, shadowColor: mode === 'TABATA' && tabataState === 'WORK' ? theme.colors.danger : theme.colors.primary, shadowOffset: { width: 0, height: 0 }, shadowOpacity: theme.isDark ? 0.5 : 0.3, shadowRadius: 20, elevation: 10 },
    timerBorder: { width: '100%', height: '100%', borderRadius: 125, padding: 4, justifyContent: 'center', alignItems: 'center' },
    timerInner: { width: '100%', height: '100%', borderRadius: 125, backgroundColor: theme.colors.bg, justifyContent: 'center', alignItems: 'center' },
    ...
    presetBtn: { paddingVertical: 10, paddingHorizontal: 15, backgroundColor: theme.colors.glass, borderRadius: 12, borderWidth: 1, borderColor: theme.colors.border },
    ...
    configCard: { width: '90%', backgroundColor: theme.colors.glass, padding: 20, borderRadius: 20, borderWidth: 1, borderColor: theme.colors.border, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 2 },
    ...
    input: { backgroundColor: theme.isDark ? 'rgba(255,255,255,0.1)' : '#f0f0f0', width: 80, padding: 8, borderRadius: 8, color: theme.colors.text, textAlign: 'center', fontWeight: 'bold' },
  });
            ...
            {['REST', 'TABATA', 'STOPWATCH'].map((m) => (
                ...
                >
                    <Text style={[styles.tabText, mode === m && styles.tabTextActive]}>
                        {m === 'REST' ? t('timer.mode_rest') : m === 'STOPWATCH' ? 'CHRONO' : t('timer.mode_tabata')}
                    </Text>
                </TouchableOpacity>
            ))}
        </View>
        ...
        <ScrollView contentContainerStyle={styles.content}>
            ...
            {mode === 'TABATA' && (
                <View style={styles.tabataInfo}>
                    <Text style={styles.roundText}>{t('timer.rounds')} {round} / {config.rounds}</Text>
            ...
            {mode === 'TABATA' && !isActive && (
                <View style={styles.configCard}>
                    ...
                    <View style={styles.inputRow}>
                        <Text style={styles.inputLabel}>{t('timer.work')} (s)</Text>
                        <TextInput 
                            style={styles.input} 
                    ...
                    <View style={styles.inputRow}>
                        <Text style={styles.inputLabel}>{t('timer.rest')} (s)</Text>
                        <TextInput 
                            style={styles.input} 
                            keyboardType="numeric" 
                    ...
                    <View style={styles.inputRow}>
                        <Text style={styles.inputLabel}>{t('timer.rounds')}</Text>
                        <TextInput 
                            style={styles.input} 
                            keyboardType="numeric" 
