...
export default function BodyBatteryScreen() {
  ...
  const [status, setStatus] = useState('CHARGÉ');
  
  const [factors, setFactors] = useState({ 
      sleep: { val: 0, label: '?' }, 
      stress: { val: 0, label: '?' }, 
      activity: { val: 0, label: 'Non' } 
  });
  ...
  const calculateBodyBattery = async () => {
    ...
    try {
        ...
        if (score >= 80) setStatus("OPTIMAL");
        else if (score >= 50) setStatus("MODÉRÉ");
        else if (score >= 30) setStatus("FATIGUÉ");
        else setStatus("ÉPUISÉ");

        await saveBodyBattery(score, session.user.id, today);

    } catch (e) {
        console.log("Erreur calcul battery", e);
    } finally {
        setLoading(false);
    }
  };
  ...
  const saveBodyBattery = async (val: number, userId: string, dateStr: string) => {
      try {
        const { data: existing } = await supabase
            ...
            .select('id')
            .eq('user_id', userId)
            .eq('type', 'body_battery')
            .eq('date', dateStr)
            .maybeSingle();

        if (existing) {
             await supabase.from('body_metrics').update({ value: val }).eq('id', existing.id);
        } else {
  ...
  const styles = StyleSheet.create({
    ...
    batteryOutline: {
        ...
        borderRadius: 30, borderWidth: 4, borderColor: theme.colors.border,
        justifyContent: 'flex-end', padding: 6,
        backgroundColor: theme.colors.glass,
        shadowColor: currentColor, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.2, shadowRadius: 20, elevation: 5
    },
    ...
    factorCard: { 
        ...
        backgroundColor: theme.colors.glass, borderRadius: 20, 
        marginHorizontal: 5, borderWidth: 1, borderColor: theme.colors.border,
    ...
    tipsCard: { 
        ...
        borderWidth: 1, borderColor: theme.colors.primary + '50', marginTop: 10
    },
    ...
    <View style={styles.container}>
      ...
      {theme.isDark && (
        <View style={styles.auroraBg}>
            <View style={[styles.blob, { top: 100, right: -50, backgroundColor: 'rgba(34, 197, 94, 0.15)' }]} />
        </View>
      )}
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>BODY BATTERY</Text>
            <View style={{ width: 40 }} />
        </View>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.batteryContainer}>
                ...
                <View style={styles.batteryOutline}>
                    <LinearGradient 
                        colors={batteryLevel > 30 ? BATTERY_GRADIENT : [theme.colors.danger, '#b91c1c']} 
                        style={[styles.batteryFill, { height: `${batteryLevel}%` }]} 
                    />
            ...
            <Text style={styles.sectionTitle}>FACTEURS DU JOUR</Text>
            <View style={styles.factorsGrid}>
                <View style={styles.factorCard}>
                    <MaterialCommunityIcons name="bed-clock" size={24} color="#8b5cf6" style={styles.factorIcon} />
                    <Text style={styles.factorValue}>{factors.sleep.label}</Text>
                    <Text style={styles.factorLabel}>Sommeil</Text>
                </View>
                
                <View style={styles.factorCard}>
                    <MaterialCommunityIcons name="brain" size={24} color="#f59e0b" style={styles.factorIcon} />
                    <Text style={styles.factorValue}>{factors.stress.label}</Text>
                    <Text style={styles.factorLabel}>Stress</Text>
                </View>

                <View style={styles.factorCard}>
                    <MaterialCommunityIcons name="dumbbell" size={24} color="#ef4444" style={styles.factorIcon} />
                    <Text style={styles.factorValue}>{factors.activity.label}</Text>
                    <Text style={styles.factorLabel}>Sport</Text>
                </View>
            </View>

            <View style={styles.tipsCard}>
                <Text style={styles.tipsTitle}>ANALYSE IA</Text>
                <Text style={styles.tipsText}>
                    {batteryLevel > 70 
                        ? "Votre énergie est optimale. C'est le moment idéal pour une séance d'entraînement intense ou une tâche complexe." 
                        : batteryLevel > 30 
                        ? "Niveau d'énergie modéré. Privilégiez un entraînement d'endurance ou de maintien." 
                        : "Batterie faible. Repos recommandé. Évitez le stress et privilégiez le sommeil ce soir."}
                </Text>
            </View>
                ...
                <View key={i} style={styles.historyItem}>
                    ...
                    <View style={styles.historyBarContainer}>
                        <View style={styles.historyBar}>
                            <View style={[
                                styles.historyFill, 
                                { width: `${item.value}%`, backgroundColor: getBatteryColor(item.value) }
                            ]} />
                        </View>
                        <Text style={styles.historyVal}>{Math.round(item.value)}%</Text>
                    </View>
                </View>
            )) : (
