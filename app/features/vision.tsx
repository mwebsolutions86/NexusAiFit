...
const GAME_DURATION = 30; // Durée en secondes
...
export default function VisionScreen() {
  ...
  const [targetPos, setTargetPos] = useState({ top: 50, left: 50 }); // Pourcentage
  const [history, setHistory] = useState<any[]>([]);
  
  const timerRef = useRef<NodeJS.Timeout | null>(null);

  // Couleur du module (Cyan électrique)
  ...
  // Astuce : On utilise saveScore dans une fermeture qui capture le score actuel ou on passe le score en paramètre
  // Ici, on va utiliser une ref ou simplement appeler une fonction qui utilise l'état courant si on est sûr qu'il est à jour.
  // Pour être sûr, on va sauvegarder juste après la mise à jour de l'état dans endGame, mais attention aux fermetures.
  // Mieux : On utilise un useEffect qui surveille timeLeft == 0, ou on passe le score.
  // Simplification : On sauvegarde l'état `score` actuel.
  
  const saveScore = async () => {
      // On utilise une petite astuce pour récupérer le score à la fin (car dans setInterval le state peut être vieux)
      // Mais ici on est appelé par le setter de setTimeLeft, donc ça devrait aller, ou alors on passe par une Ref.
      // Pour faire simple et robuste :
      const finalScore = score; // Attention, si appelé depuis setInterval, score sera 0 (closure stale).
      // CORRECTION : On va gérer la fin via un useEffect sur timeLeft.
  };

  // Gestion de la fin de partie via effet pour avoir le bon Score
  useEffect(() => {
  ...
  const moveTarget = () => {
      // Position aléatoire entre 10% et 80% pour éviter les bords extrêmes
  ...
  const styles = StyleSheet.create({
    ...
    target: {
        ...
        shadowColor: MODULE_COLOR, shadowOffset: {width:0, height:0}, shadowOpacity: 0.8, shadowRadius: 10, elevation: 10
    },
    targetRing: {
        width: 40, height: 40, borderRadius: 20,
        borderWidth: 2, borderColor: '#000', opacity: 0.5
    },
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>VISION FOCUS</Text>
            <View style={{ width: 40 }} />
        </View>
            ...
            {!isPlaying && (
                <View style={styles.overlay}>
                    <TouchableOpacity onPress={startGame} style={styles.startBtn}>
                        <Text style={styles.startText}>{timeLeft === 0 ? "REJOUER" : "START"}</Text>
                    </TouchableOpacity>
                    <Text style={styles.instructionText}>Tapez les cibles le plus vite possible !</Text>
                </View>
            )}
        ...
        <View style={styles.historyContainer}>
            <Text style={styles.historyTitle}>MEILLEURS SCORES</Text>
            {history.map((item, i) => (
