...
export default function MoodScreen() {
  ...
  // États Neuro traduits dynamiquement
  const NEURO_STATES = [
    { id: 1, label: t('modules.mood.states.discharged'), color: '#475569', speed: 3000, icon: 'battery-10', desc: 'Épuisement / Apathie', chem: { dopamine: 10, serotonin: 20, cortisol: 30, adrenaline: 10 } },
    { id: 2, label: t('modules.mood.states.anxious'), color: '#ef4444', speed: 400, icon: 'flash-alert', desc: 'Stress / Surcharge', chem: { dopamine: 40, serotonin: 10, cortisol: 95, adrenaline: 90 } },
    { id: 3, label: t('modules.mood.states.balance'), color: '#22c55e', speed: 2000, icon: 'leaf', desc: 'Calme / Stabilité', chem: { dopamine: 50, serotonin: 80, cortisol: 20, adrenaline: 30 } },
    { id: 4, label: t('modules.mood.states.flow'), color: '#8b5cf6', speed: 1000, icon: 'brain', desc: 'Concentration / Créativité', chem: { dopamine: 85, serotonin: 60, cortisol: 40, adrenaline: 50 } },
    { id: 5, label: t('modules.mood.states.hyped'), color: '#eab308', speed: 600, icon: 'lightning-bolt', desc: 'Euphorie / Haute Énergie', chem: { dopamine: 95, serotonin: 70, cortisol: 30, adrenaline: 90 } },
  ];

  const [selectedState, setSelectedState] = useState(NEURO_STATES[2]); 
  ...
  useEffect(() => { fetchHistory(); startPulse(); updateNeuroBars(); }, [selectedState]);

  const startPulse = () => {
      pulseAnim.stopAnimation(); pulseAnim.setValue(1);
      Animated.loop(Animated.sequence([Animated.timing(pulseAnim, { toValue: 1.15, duration: selectedState.speed, easing: Easing.inOut(Easing.ease), useNativeDriver: true }), Animated.timing(pulseAnim, { toValue: 1, duration: selectedState.speed, easing: Easing.inOut(Easing.ease), useNativeDriver: true })])).start();
  ...
  const fetchHistory = async () => {
      try {
        ...
        if (!session) return;
        const { data } = await supabase.from('body_metrics').select('date, value').eq('user_id', session.user.id).eq('type', 'mood_level').order('created_at', { ascending: false }).limit(5);
  ...
  const saveMood = async () => {
      ...
      try {
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) return;
        await supabase.from('body_metrics').insert({ user_id: session.user.id, type: 'mood_level', value: selectedState.id, date: new Date().toISOString().split('T')[0] });
        await fetchHistory();
  ...
  const NeuroBar = ({ label, anim, color }: any) => (
      <View style={styles.neuroRow}>
          ...
          <View style={styles.barTrack}><Animated.View style={[styles.barFill, { backgroundColor: color, width: anim.interpolate({ inputRange: [0, 100], outputRange: ['0%', '100%'] }) }]} /></View>
      </View>
  );
  ...
  const styles = StyleSheet.create({
    ...
    orbContainer: { width: 200, height: 200, justifyContent: 'center', alignItems: 'center', position: 'relative' },
    orbCore: { width: 120, height: 120, borderRadius: 60, position: 'absolute', zIndex: 2 },
    orbGlow: { width: 200, height: 200, borderRadius: 100, position: 'absolute', zIndex: 1, opacity: 0.5 },
    orbRing: { width: 240, height: 240, borderRadius: 120, borderWidth: 2, borderColor: theme.colors.border, borderStyle: 'dashed', position: 'absolute', zIndex: 0 },
    ...
    neuroContainer: { backgroundColor: theme.colors.glass, padding: 20, borderRadius: 20, borderWidth: 1, borderColor: theme.colors.border, marginBottom: 30 },
    ...
    selectorBtn: { width: 50, height: 50, borderRadius: 15, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: theme.colors.border, backgroundColor: theme.colors.glass },
  ...
  return (
    <View style={styles.container}>
      ...
      {theme.isDark && (<View style={styles.auroraBg}><View style={[styles.blob, { top: 100, left: -50, backgroundColor: selectedState.color + '20' }]} /><View style={[styles.blob, { bottom: 0, right: -50, backgroundColor: selectedState.color + '20' }]} /></View>)}
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
            <TouchableOpacity onPress={() => router.back()} style={styles.backBtn}>
                <MaterialCommunityIcons name="arrow-left" size={24} color={theme.colors.text} />
            </TouchableOpacity>
            <Text style={styles.headerTitle}>{t('modules.mood.title')}</Text>
        </View>
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            <View style={styles.visualizerContainer}>
                ...
                <Animated.View style={[styles.orbContainer, { transform: [{ scale: pulseAnim }] }]}>
                    <LinearGradient colors={[selectedState.color, 'transparent']} style={styles.orbGlow} start={{x:0.5, y:0.5}} end={{x:1, y:1}} />
            ...
            <View style={styles.selectorContainer}>
                {NEURO_STATES.map((state) => (
                    <TouchableOpacity key={state.id} style={[styles.selectorBtn, selectedState.id === state.id && { borderColor: state.color, backgroundColor: state.color + '20' }]} onPress={() => handleSelect(state)}>
                        <MaterialCommunityIcons name={state.icon as any} size={24} color={selectedState.id === state.id ? state.color : theme.colors.textSecondary} />
                    </TouchableOpacity>
                ))}
            </View>
            <View style={styles.neuroContainer}>
                <Text style={styles.neuroTitle}>{t('modules.mood.neuro_title')}</Text>
            ...
            <TouchableOpacity style={styles.saveBtn} onPress={saveMood} disabled={loading}>
                <LinearGradient colors={[selectedState.color, '#000000']} start={{x:0, y:0}} end={{x:1, y:1}} style={styles.btnGradient}>
                    {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>{t('modules.mood.btn_calibrate')}</Text>}
                </LinearGradient>
            </TouchableOpacity>
            <Text style={{color: theme.colors.textSecondary, fontSize: 10, fontWeight: 'bold', marginBottom: 10, marginLeft: 5}}>{t('modules.mood.history_title')}</Text>
            {history.map((item, i) => {
