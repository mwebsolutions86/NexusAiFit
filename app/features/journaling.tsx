...
export default function JournalingScreen() {
  ...
  // DÉPLACÉ À L'INTÉRIEUR POUR TRADUCTION
  const getMoods = () => [
      { id: 'focus', icon: 'target', label: t('modules.journal.moods.focus'), color: '#3b82f6' },
      { id: 'happy', icon: 'emoticon-happy', label: t('modules.journal.moods.happy'), color: '#22c55e' },
      { id: 'neutral', icon: 'emoticon-neutral', label: t('modules.journal.moods.neutral'), color: '#94a3b8' },
      { id: 'tired', icon: 'battery-low', label: t('modules.journal.moods.tired'), color: '#eab308' },
      { id: 'anxious', icon: 'brain', label: t('modules.journal.moods.anxious'), color: '#ef4444' },
  ];
  ...
  const saveEntry = async () => {
      if (!newContent.trim()) {
          Alert.alert(t('modules.journal.alert_empty'), t('modules.journal.alert_empty_msg'));
          return;
      }
      setLoading(true);
      ...
      try {
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) return;
        await supabase.from('journal_entries').insert({ user_id: session.user.id, content: newContent.trim(), mood: selectedMood });
        setNewContent('');
  ...
  const deleteEntry = async (id: string) => {
      ...
      Alert.alert(t('modules.journal.delete_title'), t('modules.journal.delete_msg'), [
          { text: t('modules.journal.btn_cancel'), style: "cancel" },
          { text: t('modules.journal.btn_delete'), style: "destructive", onPress: async () => {
  ...
  const styles = StyleSheet.create({
    ...
    card: { backgroundColor: theme.colors.glass, borderRadius: 20, padding: 20, marginBottom: 15, borderWidth: 1, borderColor: theme.colors.border, shadowColor: theme.isDark ? 'transparent' : '#000', shadowOffset: {width:0, height:2}, shadowOpacity:0.05, shadowRadius:4, elevation:2 },
    cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 },
    dateText: { color: theme.colors.textSecondary, fontSize: 12, fontWeight: 'bold' },
    moodBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, backgroundColor: theme.colors.bg, borderWidth: 1, borderColor: theme.colors.border },
    ...
    modalContent: { backgroundColor: theme.colors.cardBg, borderTopLeftRadius: 30, borderTopRightRadius: 30, padding: 25, height: '80%', borderTopWidth: 1, borderTopColor: theme.colors.border },
    ...
    moodCircle: { width: 50, height: 50, borderRadius: 25, justifyContent: 'center', alignItems: 'center', marginBottom: 5, borderWidth: 2 },
    moodLabel: { color: theme.colors.textSecondary, fontSize: 10, fontWeight: 'bold' },
    textInput: { backgroundColor: theme.colors.bg, color: theme.colors.text, padding: 15, borderRadius: 15, flex: 1, textAlignVertical: 'top', fontSize: 16, borderWidth: 1, borderColor: theme.colors.border, marginBottom: 20 },
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>{t('modules.journal.title')}</Text>
        ...
        <ScrollView contentContainerStyle={{paddingBottom: 50}} style={styles.content} showsVerticalScrollIndicator={false}>
            {entries.length > 0 ? entries.map((item) => {
                ...
                <View style={styles.emptyState}>
                    ...
                    <Text style={styles.emptyText}>{t('modules.journal.empty')}</Text> 
                    <TouchableOpacity onPress={() => setModalVisible(true)} style={{marginTop: 20}}>
                        <Text style={{color: MODULE_COLOR, fontWeight: 'bold'}}>{t('modules.journal.write_btn')}</Text>
                    </TouchableOpacity>
                </View>
            )}
        </ScrollView>
            ...
            <View style={styles.modalOverlay}>
                <View style={styles.modalContent}>
                    <View style={styles.modalHeader}>
                        <Text style={styles.modalTitle}>{t('modules.journal.new_entry')}</Text>
                    ...
                    <View style={styles.moodSelector}>
                        {getMoods().map((m) => (
                            <TouchableOpacity key={m.id} style={[styles.moodBtn, selectedMood === m.id && styles.moodBtnActive]} onPress={() => { if(Platform.OS!=='web') Haptics.selectionAsync(); setSelectedMood(m.id); }}>
                    ...
                    <TextInput style={styles.textInput} multiline placeholder={t('modules.journal.placeholder')} placeholderTextColor={theme.colors.textSecondary} value={newContent} onChangeText={setNewContent} autoFocus />
                    <TouchableOpacity style={styles.saveBtn} onPress={saveEntry} disabled={loading}>
                        <LinearGradient colors={JOURNAL_GRADIENT} start={{x:0, y:0}} end={{x:1, y:0}} style={styles.btnGradient}>
                            {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>{t('modules.journal.save')}</Text>}
                        </LinearGradient>
                    </TouchableOpacity>
                </View>
            </View>
