...
export default function ColdScreen() {
  ...
  const [temperature, setTemperature] = useState('12'); // Degrés Celsius
  const [history, setHistory] = useState<any[]>([]);
  const timerRef = useRef<NodeJS.Timeout | null>(null);

  const MODULE_COLOR = '#0ea5e9'; // Bleu Glacial
  ...
  const saveSession = async () => {
      ...
      try {
          ...
          // On sauvegarde la durée en minutes
  ...
  const styles = StyleSheet.create({
    ...
    timerContainer: {
        ...
        borderWidth: 4, borderColor: isActive ? MODULE_COLOR : theme.colors.border,
        backgroundColor: theme.colors.glass,
        marginBottom: 30, marginTop: 20,
        shadowColor: MODULE_COLOR, shadowOpacity: isActive ? 0.4 : 0.1, shadowRadius: 20
    },
    ...
    saveBtn: { 
        ...
        backgroundColor: theme.colors.glass, borderWidth: 1, borderColor: theme.colors.success 
    },
    ...
    <View style={styles.container}>
      ...
      {theme.isDark && (
        <View style={styles.auroraBg}>
            <View style={[styles.blob, { top: 100, right: -50, backgroundColor: 'rgba(14, 165, 233, 0.15)' }]} />
        </View>
      )}
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>IMMERSION FROID</Text>
            <View style={{ width: 40 }} />
        </View>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.timerContainer}>
                ...
                <Text style={styles.timerLabel}>{isActive ? "EXPOSITION EN COURS" : "PRÊT"}</Text>
            </View>

            <View style={styles.tempContainer}>
                <MaterialCommunityIcons name="thermometer" size={20} color={theme.colors.textSecondary} style={{marginRight: 10}} />
                <Text style={styles.tempLabel}>Température Eau</Text>
                <TextInput 
            ...
            <View style={styles.controlsRow}>
                <TouchableOpacity style={styles.actionBtn} onPress={toggleTimer}>
                    <LinearGradient 
                        colors={isActive ? [theme.colors.warning, '#d97706'] : COLD_GRADIENT} 
                ...
                {!isActive && seconds > 0 && (
                    <TouchableOpacity style={styles.saveBtn} onPress={saveSession}>
                        <Text style={styles.saveText}>TERMINER</Text>
                    </TouchableOpacity>
                )}
            </View>

            <View style={styles.historyContainer}>
                <Text style={styles.sectionTitle}>HISTORIQUE D'EXPOSITION</Text>
                {history.length > 0 ? history.map((item, i) => (
                    ...
                    <Text style={{color: theme.colors.textSecondary, fontStyle:'italic'}}>Aucune session.</Text>
                )}
            </View>
