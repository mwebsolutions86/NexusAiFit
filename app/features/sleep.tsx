...
export default function SleepScreen() {
  ...
  const [quality, setQuality] = useState(3); // 1 à 5
  const [history, setHistory] = useState<any[]>([]);
  
  // Couleur du module (Violet Nuit)
  ...
  const fetchHistory = async () => {
      try {
        ...
        const { data } = await supabase
            .from('body_metrics')
            .select('date, value, id') // On pourrait stocker la qualité dans une colonne jsonb 'meta', mais on fait simple ici
  ...
  const saveSleep = async () => {
      ...
      if (!h || h < 0 || h > 24) {
          Alert.alert("Erreur", "Entrez une durée valide (ex: 7.5)");
          return;
      }
      ...
      try {
        ...
        // On sauvegarde la durée. 
        // Idéalement, on stockerait la qualité aussi, mais pour rester compatible avec la table body_metrics simple, 
        // on va juste stocker la durée. (Ou faire une moyenne pondérée si vous voulez un "score")
        ...
        setHours('');
        await fetchHistory();
        Alert.alert("Bonne nuit !", "Données de sommeil enregistrées.");
  ...
  const getSleepStatus = (h: number) => {
      if (h < 5) return { label: "CRITIQUE", color: theme.colors.danger };
      if (h < 7) return { label: "INSUFFISANT", color: theme.colors.warning };
      if (h <= 9) return { label: "OPTIMAL", color: theme.colors.success };
      return { label: "EXCESSIF", color: theme.colors.primary };
  };
  ...
  const styles = StyleSheet.create({
    ...
    input: { 
        fontSize: 48, fontWeight: '900', color: theme.colors.text, 
        borderBottomWidth: 2, borderBottomColor: MODULE_COLOR,
        textAlign: 'center', minWidth: 100, paddingBottom: 5
    },
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>ANALYSE SOMMEIL</Text>
            <View style={{ width: 40 }} />
        </View>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.mainCard}>
                ...
                <Text style={styles.questionText}>Combien d'heures avez-vous dormi cette nuit ?</Text>
                ...
                <Text style={[styles.sectionTitle, {alignSelf:'center', marginBottom:10}]}>QUALITÉ DU REPOS</Text>
                    ...
                    {[1, 2, 3, 4, 5].map((star) => (
                        <TouchableOpacity key={star} onPress={() => {
                            ...
                            <MaterialCommunityIcons 
                                ...
                                size={32} 
                                color={star <= quality ? "#fbbf24" : theme.colors.border} 
                            />
                        </TouchableOpacity>
                    ))}
                </View>
