...
export default function FastingScreen() {
  ...
  // Audio Refs (Chargement unique pour performance)
  const tickSoundRef = useRef<Audio.Sound | null>(null);
  const alarmSoundRef = useRef<Audio.Sound | null>(null);

  // Couleur du module (Turquoise)
  ...
  const playSound = async (type: 'tick' | 'alarm') => {
    try {
      const soundObj = type === 'tick' ? tickSoundRef.current : alarmSoundRef.current;
  ...
  const toggleFast = async () => {
      ...
      if (isFasting) {
          Alert.alert(
              "Rompre le jeûne ?",
              "Voulez-vous terminer votre session maintenant ?",
              [
                  { text: "Annuler", style: "cancel" },
                  { text: "Terminer", onPress: endFast }
              ]
          );
      } else {
          // Démarrage
          ...
          playSound('tick'); // Son de démarrage
      }
  };

  const endFast = async () => {
      setLoading(true);
      playSound('alarm'); // Son de fin
      ...
      try {
          ...
          Alert.alert("Jeûne Terminé", `Bravo ! Vous avez jeûné ${formatTime(elapsed)}.`);
  ...
  const styles = StyleSheet.create({
    ...
    circleOutline: {
        ...
        shadowColor: isFasting ? MODULE_COLOR : '#000',
        shadowOffset: { width: 0, height: 0 },
        shadowOpacity: isFasting ? 0.4 : 0.1,
        shadowRadius: 20,
        elevation: 5
    },
    progressRing: {
        position: 'absolute', width: '100%', height: '100%', borderRadius: 130,
        borderWidth: 4, borderColor: isFasting ? MODULE_COLOR : 'transparent',
        opacity: 0.3
    },
    
    // CORRECTION TAILLES POLICE POUR ÉVITER LE DÉBORDEMENT
    timerValue: { 
        fontSize: 42, // Réduit
        fontWeight: '900', 
    ...
    timerLabel: { 
        fontSize: 12, 
        fontWeight: 'bold', 
        color: isFasting ? MODULE_COLOR : theme.colors.textSecondary, 
        letterSpacing: 1.5, 
        marginTop: 5,
        textAlign: 'center',
    ...
    protocolCard: { 
        width: 140, padding: 15, marginRight: 10, 
    ...
    <View style={styles.container}>
      ...
      {theme.isDark && (
        <View style={styles.auroraBg}>
            <View style={[styles.blob, { top: 100, right: -50, backgroundColor: 'rgba(20, 184, 166, 0.15)' }]} />
        </View>
      )}
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>JEÛNE INTERMITTENT</Text>
            <View style={{ width: 40 }} />
        </View>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.timerContainer}>
                <View style={styles.circleOutline}>
                    ...
                    <MaterialCommunityIcons 
                        name={isFasting ? "timer-sand" : "food-apple-outline"} 
                        size={32} 
                        color={isFasting ? MODULE_COLOR : theme.colors.textSecondary} 
                    ...
                    <Text style={styles.timerLabel}>
                        {isFasting ? "EN JEÛNE" : "FENÊTRE ALIMENTAIRE"}
                    </Text>

                    {isFasting && (
                        <View style={styles.targetBadge}>
                            <Text style={styles.targetText}>OBJECTIF : {targetHours}H ({percentage}%)</Text>
                        </View>
                    )}
                </View>
            </View>
            ...
            <TouchableOpacity style={styles.actionBtn} onPress={toggleFast} disabled={loading}>
                <LinearGradient 
                    colors={isFasting ? [theme.colors.danger, '#ef4444'] : FAST_GRADIENT} 
                    start={{x:0, y:0}} end={{x:1, y:0}} 
                    style={styles.btnGradient}
                >
                    {loading ? <ActivityIndicator color="#fff" /> : (
                        <Text style={styles.btnText}>{isFasting ? "ROMPRE LE JEÛNE" : "DÉMARRER LE JEÛNE"}</Text>
                    )}
                </LinearGradient>
            </TouchableOpacity>
            ...
            {!isFasting && (
                <>
                    <Text style={styles.sectionTitle}>CHOISIR UN PROTOCOLE</Text>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.protocolsScroll}>
                        {PROTOCOLS.map((p) => (
                            <TouchableOpacity 
                                key={p.id} 
                                style={[styles.protocolCard, targetHours === p.id && styles.protocolActive]}
                                onPress={() => {
                ...
                <View key={i} style={styles.historyItem}>
                    <View style={{flexDirection:'row', alignItems:'center'}}>
                        ...
                        <Text style={styles.historyDate}>
                            {new Date(item.created_at).toLocaleDateString(undefined, {weekday:'short', day:'numeric'})}
                        </Text>
                    </View>
                    <Text style={styles.historyVal}>{item.value}h</Text>
                </View>
            )) : (
                <Text style={{color: theme.colors.textSecondary, fontStyle:'italic', marginLeft: 5}}>Aucun jeûne enregistré.</Text>
            )}
