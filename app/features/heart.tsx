...
export default function HeartScreen() {
  ...
  const [scanning, setScanning] = useState(false); // État de mesure/scan
  ...
  // --- SIMULATION CAPTEUR / MONTRE ---
  const startMeasurement = async () => {
      setScanning(true);
      setBpm(null); // Reset affichage
      
      if (Platform.OS !== 'web') Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);

      // ICI : C'est là qu'on appellerait Apple HealthKit ou Google Health Connect
      // Pour l'instant, on simule une mesure de 3 secondes
      setTimeout(() => {
          // Génère un BPM réaliste entre 60 et 90 pour la démo
  ...
  const completeMeasurement = async (measuredVal: number) => {
      ...
      // Sauvegarde automatique après la mesure
  ...
  const saveHeartRate = async (val: number) => {
      ...
      try {
        ...
        await fetchHistory();
        Alert.alert("Mesure Réussie", `${val} BPM enregistré dans votre profil.`);
  ...
  const zones = [
      { id: 1, label: 'Échauffement', range: '50-60%', min: Math.round(maxHeartRate * 0.5), max: Math.round(maxHeartRate * 0.6), color: '#94a3b8' },
      { id: 2, label: 'Brûle Graisse', range: '60-70%', min: Math.round(maxHeartRate * 0.6), max: Math.round(maxHeartRate * 0.7), color: '#4ade80' },
      { id: 3, label: 'Aérobie', range: '70-80%', min: Math.round(maxHeartRate * 0.7), max: Math.round(maxHeartRate * 0.8), color: '#facc15' },
      { id: 4, label: 'Anaérobie', range: '80-90%', min: Math.round(maxHeartRate * 0.8), max: Math.round(maxHeartRate * 0.9), color: '#f97316' },
      { id: 5, label: 'Maximal', range: '90-100%', min: Math.round(maxHeartRate * 0.9), max: maxHeartRate, color: '#ef4444' },
  ];
  ...
  const styles = StyleSheet.create({
    ...
    mainCard: {
        ...
        borderWidth: 1, borderColor: scanning ? '#fff' : MODULE_COLOR,
    ...
    pulseContainer: {
        ...
        marginBottom: 20, borderWidth: 2, borderColor: scanning ? '#fff' : MODULE_COLOR
    },
    ...
    <View style={styles.container}>
      ...
      {theme.isDark && (
        <View style={styles.auroraBg}>
            <View style={[styles.blob, { top: 100, right: -50, backgroundColor: 'rgba(239, 68, 68, 0.15)' }]} />
        </View>
      )}
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>CARDIO FRÉQUENCE</Text>
            <View style={{ width: 40 }} />
        </View>
                    ...
                    {scanning ? (
                        ...
                        <>
                             {bpm ? (
                                ...
                                <MaterialCommunityIcons name="watch-variant" size={48} color={MODULE_COLOR} />
                             )}
                        </>
                    )}
                </View>
                ...
                <TouchableOpacity style={styles.scanBtn} onPress={startMeasurement} disabled={scanning || loading}>
                    <LinearGradient colors={scanning ? ['#555', '#777'] : HEART_GRADIENT} start={{x:0, y:0}} end={{x:1, y:0}} style={styles.btnGradient}>
                        ...
                        <Text style={styles.btnText}>
                            {scanning ? "MESURE EN COURS..." : (bpm ? "NOUVELLE MESURE" : "LANCER SYNC WATCH")}
                        </Text>
                    </LinearGradient>
                </TouchableOpacity>
                
                <Text style={{color: theme.colors.textSecondary, fontSize: 10, marginTop: 15, textAlign:'center'}}>
                    Utilise les capteurs de votre appareil ou synchronise avec Google Fit / Apple Health.
                </Text>
            </View>

            <Text style={styles.sectionTitle}>ZONES D'ENTRAÎNEMENT (Max: {maxHeartRate} BPM)</Text>
            {zones.map((zone) => (
                <View key={zone.id} style={styles.zoneItem}>
                    <View style={[styles.zoneBadge, { backgroundColor: zone.color }]}>
                        <Text style={styles.zoneId}>Z{zone.id}</Text>
                    </View>
                    <View style={styles.zoneInfo}>
                        <Text style={styles.zoneLabel}>{zone.label}</Text>
                        <Text style={styles.zoneRange}>{zone.range} FCM</Text>
                    </View>
                    <Text style={styles.zoneValues}>{zone.min}-{zone.max}</Text>
                </View>
            ))}

            <Text style={[styles.sectionTitle, {marginTop: 20}]}>HISTORIQUE RHR</Text>
            {history.length > 0 ? history.map((item, i) => (
                <View key={i} style={styles.historyItem}>
                    <Text style={styles.historyDate}>{new Date(item.date).toLocaleDateString()}</Text>
                    <Text style={styles.historyVal}>{item.value} BPM</Text>
                </View>
            )) : (
