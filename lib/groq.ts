...
// --- 4. GÉNÉRATEUR MEAL PREP (CHEF PRIVÉ) ---
export async function generateMealPrepIdeas(preferences: string): Promise<any | null> {
  if (!GROQ_API_KEY) return null;

  const systemPrompt = `
    TU ES UN CHEF GASTRONOMIQUE ET NUTRITIONNISTE SPORTIF.
    Ta mission : Créer 3 recettes de "Meal Prep" (repas préparés à l'avance) haut de gamme, saines et pratiques.
    
    CONTRAINTES :
    - Les recettes doivent bien se conserver (frigo/congélateur).
    - Ingrédients de qualité mais accessibles.
    - Format JSON STRICT.
    
    DEMANDE UTILISATEUR : "${preferences}"
    
    FORMAT JSON ATTENDU :
    {
      "recipes": [
        {
          "title": "Nom de la recette (ex: Poulet Basquaise Gastronomique)",
          "calories": 600,
          "macros": { "p": 40, "c": 50, "f": 20 },
          "ingredients": ["200g Poulet", "50g Riz", "1 Poivron"],
          "steps": ["Couper...", "Cuire..."],
          "storage": "3 jours au frigo. Réchauffer doux."
        }
      ]
    }
  `;
